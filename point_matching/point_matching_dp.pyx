...
from point_matching.point_matching_arb_wrap import _get_l_normalized, _get_victor_miller_normalization
...
cpdef get_V_tilde_matrix_b_cuspform_dp(S,int M,double Y):
    ...
    normalization, imposed_zeros = _get_victor_miller_normalization(S,True)
    V = np.zeros(shape=(nc*M,nc*M),dtype=np.complex_)
