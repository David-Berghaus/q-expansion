...
from point_matching.point_matching_arb_wrap import digits_to_bits, get_V_tilde_matrix_b_arb_wrap, get_coefficients_ir_arb_wrap

def run_unit_tests_ir_arb_wrap():
    test_get_coefficients_ir_arb_wrap()
    test_get_coefficients_ir_arb_wrap2()

def test_get_coefficients_ir_arb_wrap():
    S = AutomorphicFormSpace(Gamma0(8),4) #This example contains some empty coordinate lists
    C = get_coefficients_ir_arb_wrap(S,35)._get_mcbd(53)
    assert abs(C[0][0]) < RBF(1e-12)
    assert abs(C[1][0]-(-4))/abs(C[1][0]+(-4)) < RBF(1e-12)

    print("test_get_coefficients_ir_arb_wrap ok")

def test_get_coefficients_ir_arb_wrap2():
    ...
    print("test_get_coefficients_ir_arb_wrap for multiplicity two non-congruence ok")